...
class ewduwpmNotifications {
	...
	public function send_scheduled_emails() {
		...
		foreach ( $scheduled_emails as $key => $email ) {
			...
			{
				$this->email_all_users( $email['params'] );
			}
			elseif ( array_key_exists( 'send_type', $email ) && $email['send_type'] == 'list' )
			{
				$this->email_user_list( $email['params'] );
			}
			else
			{
				$this->email_user( $email['params'] );
			}
	...
	public function send_email( $params ) {
		...
		if ( empty( $params['email_content'] ) and empty( $params['email_title'] ) ) { return false; }

		$params['email_title'] = empty( $params['email_title'] ) ? '' : $params['email_title'];
		$params['email_content'] = empty( $params['email_content'] ) ? '' : $params['email_content'];

		$this->add_email_filters();

		$params['unique_identifier'] = ewd_random_string( 20 );

		$email_content = $this->process_email_content( $params['email_content'], $params );
		
		$email_title = apply_filters( 'ewd_uwpm_title', stripslashes( $params['email_title'] ), array() );
