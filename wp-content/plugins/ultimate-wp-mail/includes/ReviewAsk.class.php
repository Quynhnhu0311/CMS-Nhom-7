...
class ewduwpmReviewAsk {
	...
	public function enqueue_review_ask_scripts() {
		...
		wp_localize_script(
			'ewd-uwpm-review-ask-js',
			'ewd_uwpm_review_ask',
			array(
				'nonce' => wp_create_nonce( 'ewd-uwpm-review-ask-js' )
			)
		);
	}
	...
	public function hide_review_ask() {
		...
		if ( ! check_ajax_referer( 'ewd-uwpm-review-ask-js', 'nonce' ) ) {
	...
	public function send_feedback() {
		...
		$ewd_uwpm_controller
			->emailQueue
			->push_to_queue( array(
				'to'      => 'contact@etoilewebdesign.com',
				'subject' => 'UWPM Feedback - Dashboard Form',
				'message' => $feedback,
				'headers' => $headers
			) )
			->save()
